<?php   
   ...
   // Démarrage de la session 
    ...
    if(!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['new_password_retype'])){
        ...
        $check_password  = $db->prepare('SELECT password FROM students WHERE token = :token');
        ...
        if(password_verify($current_password, $data_password['password']))
        {
            ...
            if($new_password === $new_password_retype){
                ...
                $update = $db->prepare('UPDATE students SET password = :password WHERE token = :token');
                $update->execute(array(
                    "password" => $new_password,
                    "token" => $_SESSION['user']
                ));
                // On redirige
                header('Location: ../mon_espace.php?err=success_password');
                die();
            }
        }
        else{
            header('Location: ../mon_espace.php?err=current_password');
            die();
        }
